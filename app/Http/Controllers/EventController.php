...
class EventController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse { abort_unless($request->user()->can('create', Event::class), 403); $data = $this->validated($request); if ($request->user()->isAdmin()) { $request->validate(['customer_id' => ['required', 'exists:customers,id']]); $this->assertCapacityCompatible(isset($data['event_package_id']) ? EventPackage::find($data['event_package_id']) : null, $data['guest_capacity'] ?? null, 0); } else { $data['customer_id'] = $request->user()->customer_id; unset($data['event_package_id'], $data['guest_capacity']); } $data['status'] = 'draft'; $event = Event::create($data); $event->members()->syncWithoutDetaching([$request->user()->id => ['role' => 'owner']]); return to_route('events.show', $event)->with('success', 'Event created.'); }
