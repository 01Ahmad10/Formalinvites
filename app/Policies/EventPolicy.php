...
class EventPolicy
{
    public function viewAny(User $user): bool { return in_array($user->role, ['admin', 'support', 'customer'], true); }
    public function view(User $user, Event $event): bool { return $user->isAdmin() || $user->isSupport() || ($user->customer_id === $event->customer_id && $event->members()->whereKey($user)->exists()); }
