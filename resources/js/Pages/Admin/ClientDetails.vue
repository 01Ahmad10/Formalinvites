    ...
    <AuthenticatedLayout>
        ...
        <div class="fe-page fe-page-standard space-y-6">
            <div class="grid gap-6 lg:grid-cols-3">
                <Card class="lg:col-span-1"><h2 class="fe-section-title">Client Information</h2><dl class="mt-4 grid gap-3 text-sm"><div><dt class="text-[color:var(--fe-text-muted)]">Primary contact</dt><dd class="mt-1 font-medium">{{ customer.contact_name || 'Not provided' }}</dd></div><div><dt class="text-[color:var(--fe-text-muted)]">Email</dt><dd class="mt-1 break-words">{{ customer.email || 'No email' }}</dd></div><div><dt class="text-[color:var(--fe-text-muted)]">Phone</dt><dd class="mt-1">{{ customer.phone || 'No phone' }}</dd></div><div><dt class="text-[color:var(--fe-text-muted)]">Invitation allowance</dt><dd class="mt-1 font-medium">{{ allowance.used_events }} used · {{ allowance.remaining_events }} remaining of {{ allowance.allowed_events }}</dd></div></dl><form class="mt-5 border-t border-[color:var(--fe-border)] pt-4" @submit.prevent="allowanceForm.put(route('admin.customers.update', customer.id))"><label class="text-sm">Allowed invitations<input v-model="allowanceForm.allowed_events" type="number" :min="allowance.used_events" required class="mt-1 w-full" /></label><ValidationSummary :errors="allowanceForm.errors" /><Button type="submit" variant="secondary" :loading="allowanceForm.processing" class="mt-3">Save allowance</Button></form></Card>
