...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <div class="fe-page fe-page-standard space-y-6">
            ...
            <div v-if="coupons.length" class="fe-table-wrap"><table class="fe-table min-w-[48rem]"><thead><tr><th>Code</th><th>Discount</th><th>Validity</th><th>Usage</th><th>Status</th><th class="text-right">Action</th></tr></thead><tbody><tr v-for="coupon in coupons" :key="coupon.id"><td><p class="font-semibold">{{ coupon.code }}</p><p v-if="coupon.description" class="mt-1 text-xs text-[color:var(--fe-text-muted)]">{{ coupon.description }}</p></td><td class="font-medium">{{ couponDiscount(coupon.discount_type, coupon.discount_value) }}</td><td>{{ coupon.starts_at ? date(coupon.starts_at) : 'Any time' }}<span class="block text-xs text-[color:var(--fe-text-muted)]">{{ coupon.expires_at ? `Ends ${date(coupon.expires_at)}` : 'No expiry' }}</span></td><td>{{ coupon.usage_limit || 'Unlimited' }}</td><td><Badge :tone="couponTone(coupon)">{{ couponState(coupon) }}</Badge></td><td class="text-right"><Button type="button" variant="ghost" @click="edit(coupon)">Edit</Button></td></tr></tbody></table></div><EmptyState v-else title="No coupons yet." message="Create a coupon when you want to offer a discount." />
