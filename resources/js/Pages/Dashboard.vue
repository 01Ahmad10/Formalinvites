...
type Analytics = {
    ...
    upcoming: { id: number; title: string; client: string | null; template_name: string | null; date: string | null; type: string | null; capacity: number | null; status: string; families: number; responded_families: number; response_rate: number; confirmed_attendees: number }[];
    attention: { kind: 'event' | 'customer'; id: number; title: string; message: string }[];
};
...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <div v-if="isAdmin && analytics" class="fe-page fe-page-wide">
            ...
            <section class="mt-6 grid items-start gap-6 lg:grid-cols-2">
                <article class="fe-card min-w-0 p-5"><div class="flex items-center justify-between gap-3"><h2 class="fe-section-title">Upcoming Events</h2><Link :href="route('events.index')" class="text-sm underline">View events</Link></div><div v-if="analytics.upcoming.length" class="mt-4 space-y-3"><article v-for="event in analytics.upcoming" :key="event.id" class="rounded border border-[color:var(--fe-border)] p-3"><div class="flex flex-wrap justify-between gap-2"><div><p class="font-semibold">{{ event.title }}</p><p class="text-sm text-[color:var(--fe-text-secondary)]">{{ event.date }} · {{ event.client || 'Client not set' }}</p><p class="text-sm text-[color:var(--fe-text-muted)]">{{ event.type }} · {{ event.template_name || 'No template selected' }} · {{ event.capacity ?? '—' }} guests · {{ event.status }}</p><p class="mt-1 text-xs text-[color:var(--fe-text-muted)]">{{ event.responded_families }}/{{ event.families }} families responded · {{ event.confirmed_attendees }} confirmed attendees</p></div><div class="flex gap-2"><Link :href="route('events.show', event.id)" class="fe-btn fe-btn-secondary">View</Link><Link :href="route('events.builder', event.id)" class="fe-btn fe-btn-secondary">Edit Invitation</Link></div></div></article></div><p v-else class="mt-4 text-sm text-[color:var(--fe-text-muted)]">No upcoming events.</p></article>
                <article class="fe-card min-w-0 p-5"><h2 class="fe-section-title">Needs Attention</h2><div v-if="analytics.attention.length" class="mt-4 space-y-3"><article v-for="item in analytics.attention" :key="`${item.kind}-${item.id}`" class="rounded border border-[color:var(--fe-border)] p-3"><p class="font-semibold">{{ item.title }}</p><p class="text-sm text-[color:var(--fe-text-secondary)]">{{ item.message }}</p><Link :href="item.kind === 'customer' ? route('admin.customers.show', item.id) : route('events.show', item.id)" class="mt-2 inline-block text-sm underline">{{ item.kind === 'customer' ? 'Review client' : 'Review event' }}</Link></article></div><p v-else class="mt-4 text-sm text-[color:var(--fe-text-muted)]">Nothing needs attention right now.</p></article>
            </section>

            <nav aria-label="Quick actions" class="mt-6 flex flex-wrap gap-2"><Link :href="route('admin.customers.index')" class="fe-btn fe-btn-primary">Add Client</Link><Link :href="route('admin.customers.index')" class="fe-btn fe-btn-secondary">Start Client Invitation</Link><Link :href="route('admin.payments.index')" class="fe-btn fe-btn-secondary">Record Payment</Link><Link :href="route('admin.templates.index')" class="fe-btn fe-btn-secondary">Manage Templates</Link></nav>
        </div>
