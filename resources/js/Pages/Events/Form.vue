...
const props = defineProps<{ event: any; customers: any[]; packages: any[]; types: string[]; statuses: string[]; isAdmin: boolean }>();
...
const form = useForm({
    customer_id: props.event?.customer_id || '', event_package_id: props.event?.event_package_id || '', title: props.event?.title || '', event_type: props.event?.event_type || props.types[0], host_name: props.event?.host_name || '', second_host_name: props.event?.second_host_name || '', description: props.event?.description || '', main_date: dateValue(props.event?.main_date), start_time: timeValue(props.event?.start_time), end_time: timeValue(props.event?.end_time), venue: props.event?.venue || '', address: props.event?.address || '', location_url: props.event?.location_url || '', rsvp_deadline: dateValue(props.event?.rsvp_deadline), status: props.event?.status || 'draft',
});
...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <form @submit.prevent="submit" class="mx-auto max-w-4xl space-y-6 p-6">
            <section class="grid gap-4 bg-white p-6 shadow sm:grid-cols-2 sm:rounded-lg">
                <div v-if="isAdmin"><label for="customer_id" class="block text-sm font-medium">Customer</label><select id="customer_id" v-model="form.customer_id" class="mt-1 block w-full rounded border-gray-300" required><option value="">Select customer</option><option v-for="customer in customers" :key="customer.id" :value="customer.id">{{ customer.name }}</option></select><InputError :message="form.errors.customer_id" class="mt-1" /></div>
                ...
                <div class="sm:col-span-2"><label for="description" class="block text-sm font-medium">Description</label><textarea id="description" v-model="form.description" rows="4" class="mt-1 block w-full rounded border-gray-300"></textarea><InputError :message="form.errors.description" class="mt-1" /></div>
            </section>

            <section class="grid gap-4 bg-white p-6 shadow sm:grid-cols-2 sm:rounded-lg"><h3 class="sm:col-span-2 text-lg font-semibold">Date & Time</h3><div><label for="main_date" class="block text-sm font-medium">Main date</label><input id="main_date" v-model="form.main_date" type="date" class="mt-1 block w-full rounded border-gray-300"><InputError :message="form.errors.main_date" class="mt-1" /></div><div><label for="rsvp_deadline" class="block text-sm font-medium">RSVP deadline</label><input id="rsvp_deadline" v-model="form.rsvp_deadline" type="date" class="mt-1 block w-full rounded border-gray-300"><p class="mt-1 text-xs text-gray-500">RSVPs remain open through the end of the selected application day.</p><InputError :message="form.errors.rsvp_deadline" class="mt-1" /></div><div><label for="start_time" class="block text-sm font-medium">Start time</label><input id="start_time" v-model="form.start_time" type="time" class="mt-1 block w-full rounded border-gray-300"><InputError :message="form.errors.start_time" class="mt-1" /></div><div><label for="end_time" class="block text-sm font-medium">End time</label><input id="end_time" v-model="form.end_time" type="time" class="mt-1 block w-full rounded border-gray-300"><InputError :message="form.errors.end_time" class="mt-1" /></div></section>
            ...
            <section class="grid gap-4 bg-white p-6 shadow sm:grid-cols-2 sm:rounded-lg"><h3 class="sm:col-span-2 text-lg font-semibold">Package & Status</h3><div v-if="isAdmin"><label for="event_package_id" class="block text-sm font-medium">Package</label><select id="event_package_id" v-model="form.event_package_id" class="mt-1 block w-full rounded border-gray-300"><option value="">No package assigned</option><option v-for="packageItem in packages" :key="packageItem.id" :value="packageItem.id">{{ packageItem.name }}</option></select><InputError :message="form.errors.event_package_id" class="mt-1" /></div><div v-else><p class="text-sm font-medium">Package</p><p class="mt-1 text-gray-600">{{ event?.package?.name || 'Not assigned — package assignment is managed by an administrator.' }}</p></div><div><label for="status" class="block text-sm font-medium">Status</label><select id="status" v-model="form.status" class="mt-1 block w-full rounded border-gray-300"><option v-for="status in statuses" :key="status" :value="status">{{ status.replaceAll('_', ' ') }}</option></select><InputError :message="form.errors.status" class="mt-1" /></div></section>
