    ...
    <AuthenticatedLayout>
        ...
        <form class="mx-auto max-w-4xl space-y-6 p-6" @submit.prevent="submit">
            ...
            <section class="grid gap-4 bg-white p-6 shadow sm:grid-cols-2 sm:rounded-lg">
                ...
                <div><label for="main_date" class="block text-sm font-medium">Event date</label><input id="main_date" v-model="form.main_date" type="date" class="mt-1 block w-full rounded border-gray-300"><InputError :message="form.errors.main_date" class="mt-1" /></div>
                ...
                <div><label for="end_time" class="block text-sm font-medium">End time <span class="text-gray-500">(optional)</span></label><input id="end_time" v-model="form.end_time" type="time" class="mt-1 block w-full rounded border-gray-300"><InputError :message="form.errors.end_time" class="mt-1" /></div>
            </section>
            ...
            <section class="grid gap-4 bg-white p-6 shadow sm:grid-cols-2 sm:rounded-lg">
                ...
                <div v-if="isAdmin"><label for="event_package_id" class="block text-sm font-medium">Pricing Package</label><select id="event_package_id" v-model="form.event_package_id" class="mt-1 block w-full rounded border-gray-300"><option value="">No package assigned</option><option v-for="packageItem in packages" :key="packageItem.id" :value="packageItem.id">{{ packageItem.name }}</option></select><InputError :message="form.errors.event_package_id" class="mt-1" /></div>
                <div v-if="isAdmin"><label for="guest_capacity" class="block text-sm font-medium">Guest Capacity</label><input id="guest_capacity" v-model="form.guest_capacity" type="number" min="1" class="mt-1 block w-full rounded border-gray-300"><p class="mt-1 text-xs text-gray-500">The purchased capacity must fit within the selected pricing Package.</p><InputError :message="form.errors.guest_capacity" class="mt-1" /></div>
