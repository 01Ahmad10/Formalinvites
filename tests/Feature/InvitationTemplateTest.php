...
class InvitationTemplateTest extends TestCase
{
    ...
    public function test_authorized_event_editor_can_assign_active_template_and_safe_overrides(): void
    {
        ...
        $this->actingAs($editor)->get(route('events.invitation.preview', $event))->assertInertia(fn (Assert $page) => $page
            ...
            ->where('invitation.settings.body_font', 'sans')
            ->where('invitation.event.main_date', 'Wednesday, August 12, 2026')
        );

        $this->actingAs($editor)->put(route('events.invitation.update', $event), [
            'template_id' => $template->id,
            'settings' => ['primary_color' => '#243B53', 'heading_font' => 'classic_serif', 'sections' => ['schedule' => false]],
        ])->assertRedirect();

        $this->assertDatabaseHas('events', ['id' => $event->id, 'template_id' => $template->id]);
        $this->assertDatabaseHas('event_template_settings', ['event_id' => $event->id]);
        $this->actingAs($editor)->get(route('events.invitation.preview', $event))->assertInertia(fn (Assert $page) => $page
            ->component('Events/InvitationPreview')
            ->where('invitation.settings.primary_color', '#243B53')
            ->where('invitation.settings.heading_font', 'classic_serif')
            ->where('invitation.settings.sections.schedule', false)
    ...
    public function test_inactive_template_cannot_be_newly_selected_but_existing_assignment_is_retained(): void
    {
        ...
        $this->actingAs($editor)->put(route('events.invitation.update', $event), ['template_id' => $inactive->id, 'settings' => ['secondary_color' => '#243B53']])->assertRedirect();
    ...
    public function test_template_switching_preserves_event_guest_and_rsvp_data_and_event_settings_are_independent(): void
    {
        ...
        $this->actingAs($editor)->put(route('events.invitation.update', $first), ['template_id' => $secondTemplate->id, 'settings' => ['primary_color' => '#123456']])->assertRedirect();
        $this->actingAs($editor)->put(route('events.invitation.update', $second), ['template_id' => $secondTemplate->id, 'settings' => ['primary_color' => '#ABCDEF']])->assertRedirect();
        ...
        $this->actingAs($editor)->get(route('events.invitation.preview', $first))->assertInertia(fn (Assert $page) => $page->where('invitation.settings.primary_color', '#123456'));
        $admin = User::factory()->create(['role' => 'admin']);
        $this->actingAs($admin)->get(route('events.invitation.preview', $second))->assertInertia(fn (Assert $page) => $page->where('invitation.settings.primary_color', '#ABCDEF'));
    }
    ...
    public function test_unsafe_or_unknown_settings_are_rejected(): void
    {
        [$event, $editor] = $this->eventWithEditor();
        $template = $this->template();

        $this->actingAs($editor)->put(route('events.invitation.update', $event), ['template_id' => $template->id, 'settings' => ['primary_color' => 'red; background:url(x)']])->assertSessionHasErrors('settings.primary_color');
        $this->actingAs($editor)->put(route('events.invitation.update', $event), ['template_id' => $template->id, 'settings' => ['heading_font' => 'https://unsafe.example/font']])->assertSessionHasErrors('settings.heading_font');
        $this->actingAs($editor)->put(route('events.invitation.update', $event), ['template_id' => $template->id, 'settings' => ['arbitrary_css' => 'body { display:none }']])->assertSessionHasErrors('settings');
    }
