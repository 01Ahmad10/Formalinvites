...
class RsvpTest extends TestCase
{
    ...
    public function test_parties_receive_unique_opaque_tokens_and_pending_rsvps(): void
    {
        $event = $this->event();
        $one = InvitationParty::create(['event_id' => $event->id, 'name' => 'One', 'maximum_party_size' => 1]);
        $two = InvitationParty::create(['event_id' => $event->id, 'name' => 'Two', 'maximum_party_size' => 1]);
        $this->assertNotSame($one->rsvp_token, $two->rsvp_token);
        $this->assertSame(64, strlen($one->rsvp_token));
        $this->assertNotSame((string) $one->id, $one->rsvp_token);
        $this->assertSame('pending', $one->rsvp->status);
        $this->get(route('public.rsvp.show', 'bad-token'))->assertNotFound();
    }

    public function test_token_regeneration_invalidates_old_token(): void
    {
        $admin = User::factory()->create(['role' => 'admin']);
        $event = $this->event();
        $party = InvitationParty::create(['event_id' => $event->id, 'name' => 'Party', 'maximum_party_size' => 1]);
        $this->publish($event);
        $old = $party->rsvp_token;
        $this->actingAs($admin)->patch(route('events.guests.rsvp-token', [$event, $party]))->assertRedirect();
        $party->refresh();
        $this->assertNotSame($old, $party->rsvp_token);
        $this->get(route('public.rsvp.show', $old))->assertNotFound();
    ...
    public function test_party_can_submit_attending_and_update_before_deadline_with_meals_and_notes(): void
    {
        ...
        $member = $party->members()->create(['first_name' => 'John', 'member_type' => 'adult']);
        $meal = $event->mealOptions()->create(['name' => 'Chicken', 'is_active' => true]);
        $this->publish($event);
        $payload = ['status' => 'attending', 'guest_message' => 'Looking forward to it', 'members' => [['id' => $member->id, 'is_attending' => true, 'event_meal_option_id' => $meal->id, 'dietary_note' => 'Nut allergy']], 'additional_guests' => [['first_name' => 'Emma', 'last_name' => 'Smith', 'member_type' => 'child', 'event_meal_option_id' => $meal->id, 'dietary_note' => 'No dairy']]];
        $this->post(route('public.rsvp.submit', $party->rsvp_token), $payload)->assertRedirect();
        $rsvp = $party->fresh()->rsvp;
        $this->assertSame('attending', $rsvp->status);
        $this->assertSame('Looking forward to it', $rsvp->guest_message);
        $this->assertCount(2, $rsvp->personResponses);
        $this->assertDatabaseHas('rsvp_person_responses', ['rsvp_id' => $rsvp->id, 'dietary_note' => 'Nut allergy', 'is_original_party_member' => true]);
        $payload['guest_message'] = 'Updated message';
        $payload['members'][0]['is_attending'] = false;
        $payload['additional_guests'] = [];
        $this->post(route('public.rsvp.submit', $party->rsvp_token), $payload)->assertRedirect();
        $this->assertSame(1, Rsvp::count());
        $this->assertSame('Updated message', $rsvp->fresh()->guest_message);
    }

    public function test_party_can_decline_and_deadline_blocks_changes(): void
    {
        $event = $this->event();
        $party = InvitationParty::create(['event_id' => $event->id, 'name' => 'Party', 'maximum_party_size' => 1]);
        $this->publish($event);
        $this->post(route('public.rsvp.submit', $party->rsvp_token), ['status' => 'not_attending', 'guest_message' => 'Sorry'])->assertRedirect();
    ...
    public function test_public_page_shares_submitted_rsvp_summary_data_and_safe_formatted_event_context(): void
    {
        ...
        $rsvp = $party->rsvp;
        $rsvp->update(['status' => 'attending', 'guest_message' => 'See you soon', 'submitted_at' => '2026-08-11 10:32:40', 'last_updated_at' => '2026-08-11 10:32:40']);
    ...
    public function test_public_submission_cannot_exceed_capacity_or_use_wrong_or_inactive_meal(): void
    {
        ...
        $party = InvitationParty::create(['event_id' => $event->id, 'name' => 'Party', 'maximum_party_size' => 1]);
        $member = $party->members()->create(['first_name' => 'John', 'member_type' => 'adult']);
        $wrong = $this->event()->mealOptions()->create(['name' => 'Wrong', 'is_active' => true]);
        $inactive = $event->mealOptions()->create(['name' => 'Inactive', 'is_active' => false]);
        $this->publish($event);
        $base = ['status' => 'attending', 'members' => [['id' => $member->id, 'is_attending' => true, 'event_meal_option_id' => $wrong->id]], 'additional_guests' => []];
        $this->post(route('public.rsvp.submit', $party->rsvp_token), $base)->assertSessionHasErrors('members');
        $base['members'][0]['event_meal_option_id'] = $inactive->id;
        $this->post(route('public.rsvp.submit', $party->rsvp_token), $base)->assertSessionHasErrors('members');
        $base['members'][0]['event_meal_option_id'] = null;
        $base['additional_guests'] = [['first_name' => 'Extra', 'member_type' => 'adult']];
        $this->post(route('public.rsvp.submit', $party->rsvp_token), $base)->assertSessionHasErrors('additional_guests');
    }

    public function test_protected_rsvp_pages_format_submission_dates_without_duplicate_initial_update(): void
    {
        $admin = User::factory()->create(['role' => 'admin']);
        $event = $this->event();
        $party = InvitationParty::create(['event_id' => $event->id, 'name' => 'Party', 'maximum_party_size' => 1]);
        $rsvp = $party->rsvp;
        $rsvp->update(['status' => 'attending', 'submitted_at' => '2026-08-11 10:08:42', 'last_updated_at' => '2026-08-11 10:08:42']);
        $this->actingAs($admin)->get(route('events.rsvps.index', $event))->assertInertia(fn (Assert $page) => $page->component('Events/Rsvps/Index')->where('parties.0.rsvp.submitted_at', 'August 11, 2026 at 10:08 AM')->where('parties.0.rsvp.last_updated_at', null));
        $rsvp->update(['last_updated_at' => '2026-08-11 10:55:00']);
        $this->actingAs($admin)->get(route('events.rsvps.show', [$event, $party]))->assertInertia(fn (Assert $page) => $page->component('Events/Rsvps/Show')->where('party.rsvp.submitted_at', 'August 11, 2026 at 10:08 AM')->where('party.rsvp.last_updated_at', 'August 11, 2026 at 10:55 AM'));
    }

    public function test_event_rsvp_summary_uses_actual_attending_people_and_permissions_are_enforced(): void
    {
        $admin = User::factory()->create(['role' => 'admin']);
        ...
        $rsvp = $party->rsvp;
        $rsvp->update(['status' => 'attending', 'submitted_at' => now(), 'last_updated_at' => now()]);
    ...
    private function event(?Customer $customer = null): Event
    {
        ...
        return Event::create(['customer_id' => $customer->id, 'event_package_id' => $package->id, 'title' => 'Event '.uniqid(), 'event_type' => 'wedding', 'host_name' => 'Host', 'status' => 'draft', 'rsvp_deadline' => now()->addWeek()->toDateString()]);
    }
